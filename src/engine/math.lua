...
function math.generate_poisson_disc_sampled_points(rs, x, y, w, h)
  ...
  local is_valid = function(x, y)
    if x >= 0 and x <= w and y >= 0 and y <= h then
      ...
      for i = sx1, sx2 do
        for j = sy1, sy2 do
          local point_index = grid:get(i, j)
  ...
  table.insert(spawn_points, {random:float(0, w), random:float(0, h)})
  while #spawn_points > 0 do
    local spawn_index = random:int(1, #spawn_points)
    local spawn_center = spawn_points[spawn_index]
    local accepted = false
    for i = 1, 30 do
      local r = random:angle()
      local d = random:float(rs, 2*rs)
      local cx, cy = spawn_center[1] + d*math.cos(r), spawn_center[2] + d*math.sin(r)
      if is_valid(cx, cy) and grid:get(math.floor(cx/cell_size), math.floor(cy/cell_size)) == 0 then
        table.insert(points, vec2(cx, cy))
        table.insert(spawn_points, {cx, cy})
        grid:set(math.floor(cx/cell_size), math.floor(cy/cell_size), #points)
        accepted = true
        break
      end
    end
...
-- r = math.angle_to(player.x, player.y, enemy.x, enemy)
function math.angle_to(x, y, px, py)
  return math.atan2(py - y, px - x)
end

-- Returns the angle from point x, y to the mouse.
-- r = math.angle_to_mouse(camera, player.x, player.y)
function math.angle_to_mouse(camera, x, y)
  local mx, my = camera:get_mouse_position()
  return math.atan2(my - y, mx - x)
end

-- Returns the angle from the mouse to this point x, y.
-- r = math.angle_from_mouse(camera, player.x, player.y)
function math.angle_from_mouse(camera, x, y)
  local mx, my = camera:get_mouse_position()
  return math.atan2(y - my, x - mx)
end

-- Returns the distance from point x, y to the mouse
-- d = math.distance_to_mouse(camera, player.x, player.y)
function math.distance_to_mouse(camera, x, y)
  local mx, my = camera:get_mouse_position()
