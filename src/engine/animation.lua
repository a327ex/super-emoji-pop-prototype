...
animation_frames = class:use()
function animation_frames:new(image, frame_w, frame_h, frames_list)
  self.source = image
...
function animation_frames:draw(layer, frame, x, y, r, sx, sy, ox, oy, z, fixed)
  draw_quad(layer, self.source.image, self.frames[frame].quad, x, y, r or 0, sx or 1, sy or sx or 1, self.frames[frame].w/2 + (ox or 0), self.frames[frame].h/2 + (oy or 0), z, fixed)
end
...
animation_logic = class:use()
function animation_logic:new(delay, frames, loop_mode, actions)
  self.delay = delay
  self.frames = frames
...
function animation_logic:update(dt)
...
-- The animation class, a mix of animation_frames and animation_logic.
-- Takes in a delay, an animation_frames object, the loop mode and a table of actions.
-- Read more about the animation_frames and animation_logic classes as everything there applies here.
animation = class:use()
function animation:new(delay, animation_frames, loop_mode, actions)
  ...
  self.actions = actions
  self.animation_logic = animation_logic(self.delay, self.animation_frames.size, self.loop_mode, self.actions)
end


function animation:update(layer, dt, x, y, r, sx, sy, ox, oy, z, fixed)
  self.animation_logic:update(dt)
  self.animation_frames:draw(layer, self.animation_logic.frame, x, y, r, sx, sy, ox, oy, z, fixed)
end
